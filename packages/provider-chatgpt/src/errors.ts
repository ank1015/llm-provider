...
export function invalidConfig(message: string): LlmError {
  return new LlmError(message, { provider: "chatgpt", kind: "invalid_config" });
}

export function invalidRequest(message: string): LlmError {
  return new LlmError(message, { provider: "chatgpt", kind: "invalid_request" });
}

export function invalidResponse(message: string, nativeError?: unknown): LlmError {
  return new LlmError(message, { provider: "chatgpt", kind: "invalid_response", nativeError });
}
...
export function httpError(response: Response, body: string): LlmError {
  ...
  const error = detail as { message?: unknown; detail?: unknown; code?: unknown; type?: unknown } | null;
  return new LlmError(
    typeof error?.message === "string" ? error.message
      : typeof error?.detail === "string" ? error.detail
      : body || response.statusText || `ChatGPT request failed with HTTP ${response.status}.`,
    {
      provider: "chatgpt", kind: "provider_error", httpStatus: response.status,
