...
export function buildResponseRequest({
...
}): { model: ChatGptModelId; input: unknown[]; [key: string]: unknown } {
  if (!CHATGPT_MODELS.some((model) => model.id === modelId)) {
    throw invalidRequest(`Unsupported ChatGPT model: ${modelId}.`);
  }

  if ("max_output_tokens" in providerOptions) {
    throw invalidRequest("ChatGPT backend does not support providerOptions.max_output_tokens.");
  }

  const options = { ...providerOptions };
  if (options.background === true) throw invalidRequest("ChatGPT background responses are unsupported by this synchronous adapter.");
  ...
  if (responsesLite) {
    ...
    options.instructions = instructions ?? DEFAULT_CHATGPT_INSTRUCTIONS;
  ...
  switch (message.role) {
    ...
    case "assistant":
      if (message.provider !== "chatgpt") {
        throw invalidRequest(`Cannot replay ${message.provider} assistant content with ChatGPT.`);
      }
      return message.content;
    case "tool_result":
      return [mapToolResult(message, tools)];
    case "custom":
      if (message.tag !== CHATGPT_CUSTOM_ITEM_TAG) {
        throw invalidRequest(`Unsupported ChatGPT custom message tag: ${message.tag}.`);
      }
...
function mapToolResult(message: ToolResultMessage, tools: readonly ToolDefinition[]) {
  ...
  if (!tool) throw invalidRequest(`Missing tool definition for ChatGPT tool result: ${message.toolName}.`);
