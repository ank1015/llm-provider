...
import { buildResponseRequest, type OpenAiModelId } from "../src/index.js";
...
describe("buildResponseRequest", () => {
  it("owns model, input, instructions and stream while preserving other options", () => {
    const providerOptions = Object.freeze({
      model: "bypass", input: ["bypass"], instructions: "bypass", stream: true,
    ...
    expect(buildResponseRequest({
      ...
      model: modelId, instructions: "Be concise", temperature: 0.2, max_output_tokens: 4000,
      input: [
  ...
  it("replays native assistant and custom items in order without validating their payloads", () => {
    ...
    const body = buildResponseRequest({
      modelId, messages: [
        { role: "assistant", provider: "openai", content: [reasoning, call] },
        { role: "custom", tag: "openai_custom_item", data: { content: [custom, null] } },
      ],
    });
  ...
  it("passes a native compaction trigger while keeping transport options out of the body", () => {
    ...
    const body = buildResponseRequest({
      modelId,
      messages: [{ role: "custom", tag: "openai_custom_item", data: { content: [trigger] } }],
  ...
  it("uses the Lite prefix and removes image details without mutating native data", () => {
    ...
    const body = buildResponseRequest({
      ...
      messages: [
        ...
        { role: "assistant", provider: "openai", content: [nativeMessage] },
        { role: "custom", tag: "openai_custom_item", data: { content: outputs } },
      ],
    });
  ...
  it("rejects unsupported models, assistant providers, tags and missing tool definitions", () => {
    expect(() => buildResponseRequest({ modelId: "unknown" as OpenAiModelId, messages: [] })).toThrow("Unsupported OpenAI model");
    expect(() => buildResponseRequest({ modelId, messages: [{ role: "assistant", provider: "chatgpt", content: [] }] })).toThrow("Cannot replay chatgpt");
    expect(() => buildResponseRequest({ modelId, messages: [{ role: "custom", tag: "application", data: {} }] })).toThrow("Unsupported OpenAI custom message tag");
